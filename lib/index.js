...
(function() {
  ...
  ttml2sbv = function(lines) {
    var groups1, groups2, i, matches1, matches2, numLines, results, text, text1, text2;
    results = [];
    ...
    while (i < numLines) {
      ...
      while (!matches1 && i < numLines) {
        matches1 = lines[i++].match(ttmlRE);
      }
      if (matches1) {
        // Get second line of caption text.
        while (!matches2 && i < numLines) {
          matches2 = lines[i++].match(ttmlRE);
        }
        groups1 = matches1 != null ? matches1.groups : void 0;
        groups2 = matches2 != null ? matches2.groups : void 0;
        text1 = decodeEntities(groups1.text);
        text2 = decodeEntities((groups2 != null ? groups2.text : void 0) || ''); // groups2 might be null.
  ...
  html2Markdown = function(html) {
    var j, len, line, lines, markdown, options, results, turndownService;
...
// keep consecutive timestamps to be tightly packed.
  ...
  markdown2Html = function(markdown) {
    var groups, html, j, len, line, lines, m, matches, ms, results, s, seconds;
    lines = markdown.split('\n');
    results = [];
    for (j = 0, len = lines.length; j < len; j++) {
      line = lines[j];
  ...
  OtrgenCommand = class OtrgenCommand extends Command {
    async run() {
      var args, html, inputFormat, lines, otr, outputFormat, result, resultsHtml, text, ts;
