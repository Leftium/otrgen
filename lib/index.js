...
(function() {
  var Command, OtrgenCommand, arg, decodeEntities, flags, fs, markdownTimestamp, options, otrTimestamp, parseBlock, tsRE, ttml2sbv, ttmlRE;
  ...
  OtrgenCommand = class OtrgenCommand extends Command {
    async run() {
      var args, html, inputFormat, lines, markdown, otr, outputFormat, resultsHtml, resultsMarkdown, text, ts;
      ...
      if (inputFormat === 'otr' || inputFormat === 'html' || inputFormat === 'htm') {
        if (inputFormat === 'otr') {
          otr = JSON.parse(text);
          html = otr.text;
        } else {
        ...
        switch (outputFormat) {
          ...
          case 'otr':
            return this.log(otr);
          case 'md':
            return this.log(markdown);
          default:
            return this.error(`Unknown output format ${outputFormat}.`);
        }
      }
    }
